...
| TBD                                 | GHCR cleanup, each trigger owns its scope: PR close deletes the branch tag with its per-arch children (`ghcr-cleanup-action`); publish prunes the manifest it dethrones by retagging; manual dispatch sweeps all untagged orphans |
