...
pnpm backend development up -d
```

This starts:

- **PostgreSQL 15** (Supabase image) published on `PGPORT` (`55432` in `.env`)
- **PostgREST v12** published on `POSTGREST_PORT` (`55433` in `.env`)
...
```bash
# WSL
pnpm backend development up -d   # from apps/builder
```
